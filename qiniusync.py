...
#
# AUTHOR = "heqingpan"
# AUTHOR_EMAIL = "heqingpan@126.com" 原作者
# 适配python3 作者 liuchunyao@126.com
...
ignore_names = [".DS_Store", ".git", ".gitignore",
                "sync.py", "README.md", "LICENSE", "test_version/.DS_Store", ".qiniu_pythonsdk_hostscache.json"]
charset = "utf8"
diff_time = 2*60
...
def sync():
    ...
    for k, f in l_key_files:
        ...
        local_keys.append(k)
        if isinstance(k, str):
            k = k.encode(charset).decode(charset)
        if k in qn_set:
    ...
    check_delete(qn_keys, local_keys)

    print("同步结束")


def check_delete(qn_keys, local_keys):
...
def down_file(key, basedir="", is_private=1, expires=3600):
    ...
    savepath = os.path.join(basedir, fpath).encode(charset).decode(charset)
